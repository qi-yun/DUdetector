...
class incStat:
    def __init__(self, Lambda, ID, init_time=0, isTypeDiff=False):
        ...
        self.cur_var = np.nan
        self.cur_std = np.nan
        self.covs = [] # a list of related incStat_cov references
...
class incStat_cov:
    ...
    def update_cov(self, ID, v, t,istypeDiff=True):  # it is assumes that incStat "ID" has ALREADY been updated with (t,v) [this si performed automatically in method incStat.insert()]
        # find incStat
        if ID == self.incStats[0].ID:   ##incS1
            inc = 0
        elif ID == self.incStats[1].ID:   ##incS2
            inc = 1
        else:
        ...
        v = self.insert_jiiter(t,istypeDiff)
    ...
    def insert_jiiter(self,t=0,isTypeDiff=True):  # t is v's arrival the timestamp
        if isTypeDiff:
            dif = t - self.lastTimestamp
            if dif > 0:
                return dif
...
class incStatDB:
    ...
    def register(self,ID,Lambda=1,init_time=0,isTypeDiff=False):
        ...
        #Retrieve incStat
        key = ID+"_"+str(Lambda)
        incS = self.HT.get(key)   ##self.HT = dict()
    ...
    # Registers covariance tracking for two streams and registers missing streams
    def register_cov(self,ID1,ID2,Lambda=1,init_time=0,isTypeDiff=False):
        ...
        ##incStats[0]=incS1,incStats[1]=incS2
        # Link incStats
    ...
    # Updates and returns current 1D stats for the given ID. Automatically registers previously unknown stream IDs
    ...
    def update_get_jitter_2D_Stats(self, ID1,ID2,t1,v1,Lambda=1,isTypeDiff=True,level=1):  #level=  1:cov,pcc  2:radius,magnitude,cov,pcc
        #retrieve/add cov tracker
        inc_cov = self.register_cov(ID1, ID2, Lambda, t1)
        # Update cov tracker
        inc_cov.update_cov(ID1,v1,t1,isTypeDiff)
        if level == 1:
            return inc_cov.get_stats1()
        else:
            return inc_cov.get_stats2()
    ...
    def update_get_jitter_1D2D_Stats(self, ID1,ID2,t1,v1,Lambda=1,isTypeDiff=True):  # weight, mean, std
        return self.update_get_1D_Stats(ID1+ID2, t1, 0, Lambda, isTypeDiff) + self.update_get_jitter_2D_Stats(ID1,ID2,t1,v1,Lambda,isTypeDiff,level=2,)
